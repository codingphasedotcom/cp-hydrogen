...
export default function Collection1({popularProductsData, columns}) {
	let  columnsClasses = 'col-sm-1 col-md-4'

	const setColumns = (function() {
		columnsClasses = 'col-sm-1 col-md-4'
		switch (columns) {
			case 1:
				columnsClasses = 'col-sm-12 col-md-12'
				break;
				...
				<div className="row">
					{popularProductsData.map((item) => {
						...
						item.variants.edges.map((edge) => {
							...
							variantColor = itemColors.push(variantColor)
						})
						...
						<div className={columnsClasses}>
							<div className="collection1__collection-item">
								<Link to={`/products/${item.handle}`} className={'collection1__product-img collection1__product-img--rectangle'}>
